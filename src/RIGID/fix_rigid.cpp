...
void FixRigid::setup(int vflag)
{
  ...
  for (i = 0; i < nlocal; i++) {
    ...
    /*NL*/ //double f2[3];vectorCopy3D(f[i],f2);f2[2]=0.;
    /*NL*/ //if(vectorMag3D(f2) > 0.) { fprintf(screen,"particle i %d, f %e %e %e\n",i,f[i][0],f[i][1],f[i][2]);
    /*NL*/ //error->one(FLERR,"end setup"); }
  }
...
void FixRigid::final_integrate()
{
  ...
  for (i = 0; i < nlocal; i++) {
    ...
    /*NL*/ //if(vectorMag3D(f[i]) > 0.) { fprintf(screen,"particle i %d, f %e %e %e\n",i,f[i][0],f[i][1],f[i][2]);
